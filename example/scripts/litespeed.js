...
let attrs=node.attributes;let attrsLen=attrs.length;if(attrs&&attrsLen){for(let x=0;x<attrsLen;x++){let pointer=(!/Edge/.test(navigator.userAgent))?x:(attrsLen-1)-x;let length=attrsLen;let attr=attrs[pointer];if(!stock[attr.nodeName]){continue;}
...
init(route);});init(state.match(window.location));}});container.get('view').add({selector:'data-ls-attr',template:false,repeat:true,controller:function(element,expression){let attrs=element.dataset['lsAttr'].trim().split(',');console.log(attrs);for(let i=0;i<attrs.length;i++){let attr=attrs[i].split('=');let key=(attr[0])?attr[0]:null;let value=(attr[1])?attr[1]:null;if(!key){return null;}
element.setAttribute(key,value);}}});container.get('view').add({selector:'data-ls-echo',template:false,repeat:true,controller:function(element,expression,filter){let def=expression.parse(element.getAttribute('data-default')||'');let filterName=element.getAttribute('data-filter')||'';let filterOptions=JSON.parse(element.getAttribute('data-filter-options')||'{}');let result=expression.parse(element.dataset['lsEcho']);result=result||def;if(filterName){result=filter.apply(result,filterName,filterOptions);}

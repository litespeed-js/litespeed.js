...
return null;}
if(service.instance===null){let instance=(typeof service.object==='function')?this.resolve(service.object):service.object;if(name!=='window'&&name!=='document'&&name!=='element'&&typeof instance==='object'&&instance!==null){instance=new Proxy(instance,{path:[],name:service.name,get:function(obj,prop){this.path.push(prop);if(typeof obj[prop]==='object'&&obj[prop]!==null){return new Proxy(obj[prop],this)}
else{return obj[prop];}},set:function(obj,prop,value){this.path.push(prop);obj[prop]=value;if(prop!=='length'){document.dispatchEvent(new CustomEvent(this.name+'.changed'));}
this.path=[];return true;},});}
...
states[states.length++]={path:path,view:view};return this;};let match=function(location){console.log('match',location);let url=location.pathname;states.sort(function(a,b){return b.path.length-a.path.length;});states.sort(function(a,b){let n=b.path.split('/').length-a.path.split('/').length;if(n!==0){return n;}
...
console.log('match click');let route=state.match(target);if(null===route){return false;}
...
init(route);return true;});window.addEventListener('popstate',function(){console.log('match popstate');let route=state.match(window.location);if(state.getPrevious()&&state.getPrevious().view&&(state.getPrevious().view.scope!==route.view.scope)){window.location.reload();return false;}
init(route);});console.log('match init');init(state.match(window.location));}});container.get('view').add({selector:'data-ls-attr',template:false,repeat:true,controller:function(element,expression){let attrs=element.dataset['lsAttr'].trim().split(',');for(let i=0;i<attrs.length;i++){let attr=attrs[i].split('=');let key=(attr[0])?expression.parse(attr[0]):null;let value=(attr[1])?expression.parse(attr[1]):null;if(!key){return null;}
element.setAttribute(key,value);}}});container.get('view').add({selector:'data-ls-bind',controller:function(element,expression,container){let echo=function(value){if(element.tagName==='INPUT'||element.tagName==='OPTION'||element.tagName==='SELECT'||element.tagName==='BUTTON'||element.tagName==='TEXTAREA'){let type=element.getAttribute('type');if('radio'===type){if(value.toString()===def){element.setAttribute('checked','checked');}
else{element.removeAttribute('checked');}}
if('checkbox'===type){if(def.includes(value.toString())){element.setAttribute('checked','checked');}
else{element.removeAttribute('checked');}}
if(element.value!==value){element.value=value;}
element.addEventListener('change',sync);element.addEventListener('keyup',sync);}
else{if(element.innerText!==value){element.innerHTML=value;}}};let sync=function(){container.path(paths[0],element.value);};let expr=element.dataset['lsBind'];let result=expression.parse(expr);let paths=expression.getPaths();for(let i=0;i<paths.length;i++){document.addEventListener(paths[i].split('.')[0]+'.changed',function(){echo(expression.parse(expr));});}
echo(result);}});container.get('view').add({selector:'data-ls-if',template:false,repeat:true,controller:function(element,expression){let result='';let syntax=element.dataset['lsIf']||'';try{result=!!(eval(expression.parse(syntax,'undefined').replace(/(\r\n|\n|\r)/gm,' ')));}
...
children[prop]=children[prop]||element.backup.cloneNode(true);element.appendChild(children[prop]);(function(index){let context=expr+'.'+index;container.set(element.dataset['lsAs'],container.path(context),true);container.set('$index',index,true);view.render(children[prop]);})(prop);}};element.template=(element.template)?element.template:(element.children.length===1)?element.children[0].innerHTML:'';if(!element.backup){element.backup=(element.children.length===1)?element.children[0]:window.document.createElement('li');}
echo();document.addEventListener(expr.split('.')[0]+'.changed',function(){echo();});}});container.get('view').add({selector:'data-ls-options',repeat:true,controller:function(element,document,expression,container){let options=expression.parse(element.dataset['lsOptions']||'{}');let key=element.dataset['key']||null;let label=element.dataset['label']||null;let placeholder=expression.parse(element.dataset['placeholder']||'');let value=(element.selectedIndex>-1)?element.options[element.selectedIndex].value:null;element.innerHTML='';if(placeholder){let child=document.createElement('option');child.value='';child.innerText=placeholder;element.appendChild(child);}

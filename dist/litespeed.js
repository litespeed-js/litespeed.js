...
element.setAttribute(key,value);}}});container.get('view').add({selector:'data-ls-bind',controller:function(element,expression,container,$prefix,$as){let echo=function(value){if(element.tagName==='INPUT'||element.tagName==='SELECT'||element.tagName==='BUTTON'||element.tagName==='TEXTAREA'){let type=element.getAttribute('type');if('radio'===type){if(value.toString()===element.value){element.setAttribute('checked','checked');}
...
element.addEventListener('change',function(){console.log(element.checked,path);container.path(path,element.checked);});}
return;}
if(element.value!==value){element.value=value;}
element.addEventListener('change',sync);element.addEventListener('keyup',sync);}
else{if(element.innerText!==value){element.innerHTML=value;}}};let sync=function(){container.path(path,element.value);};let path=element.dataset['lsBind'];let result=container.path(path);path=path.replace($as,$prefix);document.addEventListener(path.split('.')[0]+'.changed',function(){echo(container.path(path));});echo(result);}});container.get('view').add({selector:'data-ls-if',template:false,repeat:true,controller:function(element,expression){let result='';let syntax=element.dataset['lsIf']||'';try{result=!!(eval(expression.parse(syntax,'undefined').replace(/(\r\n|\n|\r)/gm,' ')));}

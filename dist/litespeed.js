...
k++;}
return false;};}
container.set('view',function(http,container){let stock={};let execute=function(view,node,container){container.set('element',node,true);container.resolve(view.controller);if(true!==view.repeat){node.removeAttribute(view.selector);}};return{stock:stock,add:function(object){if(typeof object!=='object'){throw new Error('object must be of type object');}
...
stock[object.selector]=object;return this;},render:function(element){let self=this;let list=(element)?element.childNodes:[];if(element.$lsSkip===true){list=[];}
for(let i=0;i<list.length;i++){let node=list[i];if(1!==node.nodeType){continue;}
if(node.attributes&&node.attributes.length){for(let x=0;x<node.attributes.length;x++){let pointer=(!/Edge/.test(navigator.userAgent))?x:(node.attributes.length-1)-x;let length=node.attributes.length;let attr=node.attributes[pointer];if(!stock[attr.nodeName]){continue;}
let comp=stock[attr.nodeName];if(typeof comp.template==="function"){comp.template=container.resolve(comp.template);}
...
return json;}
return{'toJson':toJson}},true);container.set('state',function(window){let states=[];let current=null;let previous=null;let getPrevious=function(){return previous;};let setPrevious=function(value){previous=value;return this;};let getCurrent=function(){return current;};let setCurrent=function(value){current=value;return this;};let getParam=function(key,def){if(key in state.params){return state.params[key];}
return def;};let getParams=function(){return state.params;};let setParam=function(key,value){state.params[key]=value;return this;};let reset=function(){state.params=getJsonFromUrl(window.location.search);};let getURL=function(){return window.location.href;};let add=function(path,view){if(typeof path!=='string'){throw new Error('path must be of type string');}
...
window.dispatchEvent(new PopStateEvent('popstate',{}));};let reload=function(){change(window.location.href);};let getJsonFromUrl=function(URL){let query;if(URL){let pos=location.href.indexOf('?');if(pos==-1)return[];query=location.href.substr(pos+1);}else{query=location.search.substr(1);}
...
catch(error){var handler=container.resolve(this.error);handler(error);}},error:function(){return function(error){console.error('error',error.message,error.stack,error.toString());}},container:container,view:container.get('view')}};container.get('view').add({selector:'data-ls-init',template:false,repeat:true,controller:function(element,window,document,view,state){let firstFromServer=(element.getAttribute('data-first-from-server')==='true');let scope={selector:'data-ls-scope',template:false,repeat:true,controller:function(){},state:true},init=function(route){window.scrollTo(0,0);if(window.document.body.scrollTo){window.document.body.scrollTo(0,0);}
...
else{element.style.removeProperty('display');element.style.removeProperty('visibility');}}});container.get('view').add({selector:'data-ls-loop',template:false,repeat:true,nested:false,controller:function(element,view,container,window){let array=container.path(element.dataset['lsLoop']);array=(!array)?[]:array;element.template=(element.template)?element.template:(element.children.length===1)?element.children[0].innerHTML:'';if(!element.backup){element.backup=(element.children.length===1)?element.children[0]:window.document.createElement('li');}
...
children[prop]=children[prop]||element.backup.cloneNode(true);element.appendChild(children[prop]);(function(index){let context=element.dataset['lsLoop']+'.'+index;container.set(element.dataset['lsAs'],container.path(context),true);container.set('$index',index,true);view.render(children[prop]);})(prop);}}});container.get('view').add({selector:'data-ls-options',repeat:true,controller:function(element,document,expression,container){let options=expression.parse(element.dataset['lsOptions']||'{}');let key=element.dataset['key']||null;let label=element.dataset['label']||null;let placeholder=expression.parse(element.dataset['placeholder']||'');let value=(element.selectedIndex>-1)?element.options[element.selectedIndex].value:null;element.innerHTML='';if(placeholder){let child=document.createElement('option');child.value='';child.innerText=placeholder;element.appendChild(child);}

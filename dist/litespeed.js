...
window.ls.container = function () {
    ...
    let set = function (name, object, singleton, watch = true) {
        ...
        stock[name] = {name: name, object: object, singleton: singleton, instance: null, watch: watch,};
    ...
    let get = function (name) {
        ...
        if (service.watch && name !== 'window' && name !== 'document' && name !== 'element' && typeof instance === 'object' && instance !== null) {
            let handler = {
                name: service.name, watch: function () {
    ...
    let path = function (path, value, as, prefix) {
        ...
        path = ((path.indexOf('.') > -1) ? path.replace(as + '.', prefix + '.') : path.replace(as, prefix)).split('.');
        let name = path.shift();
    ...
    let container = {set: set, get: get, resolve: resolve, path: path, bind: bind, stock: stock, listeners: listeners,};
    set('container', container, true, false);
    return container;
}();
window.ls.container.set('http', function (document) {
    let globalParams = [], globalHeaders = [];
    ...
    return {
        stock: stock, add: function (object) {
            ...
            let defaults = {
                ...
                }, 'template': '', 'repeat': false, 'protected': false
            };
...
window.ls.container.get('view').add({
    selector: 'data-ls-router', repeat: false, controller: function (element, window, document, view, router) {
        let firstFromServer = (element.getAttribute('data-first-from-server') === 'true');
        let scope = {
            selector: 'data-ls-scope', template: false, repeat: true, controller: function () {
            },
        };
...
window.ls.container.get('view').add({
    ...
    controller: function (element, view, http, expression, document) {
        ...
        http.get(template).then(function (element) {
            return function (data) {

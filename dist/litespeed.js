...
else{if(element.innerText!==result){element.innerHTML=result;}}}});container.get('view').add({selector:'data-ls-if',template:false,repeat:true,controller:function(element,expression){let result='';let syntax=element.dataset['lsIf']||'';try{result=!!(eval(expression.parse(syntax,'undefined').replace(/(\r\n|\n|\r)/gm,' ')));}
...
if(typeof options==='object'){Object.keys(options).map(function(x){let child=document.createElement('option');container.set('option',options[x],false,false);child.value=(key)?expression.parse(key):x;child.innerText=(label)?expression.parse(label):(typeof options[x]!=='string')?JSON.stringify(options[x]):options[x];child.selected=(child.value===value);element.appendChild(child);element.disabled=false;});}}});container.get('view').add({selector:'data-ls-rerender',template:false,repeat:true,controller:function(element,view,http,expression,document,container){let events=element.dataset['lsRerender']||'';let scope=element.dataset['scope']||'';scope=(scope)?container.get(scope):document;events=events.split(',');for(let i=0;i<events.length;i++){let event=events[i].trim();if(''===event){continue;}
